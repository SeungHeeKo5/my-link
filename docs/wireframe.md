...
본 문서는 마이링크(MyLink) 프로젝트의 전체적인 화면 구조(레이아웃)와 구성 요소들의 배치를 정의합니다. shadcn/ui 기반의 모던하고 깔끔한 데스크탑 랜딩 페이지 레이아웃을 기준으로 작성되었습니다. 모바일에서는 아래의 좌측 영역이 상단으로, 우측 영역이 하단으로 스택(Stack)되어 일렬로 나열됩니다.
...
flowchart TD
    subgraph View ["방문자용 랜딩 페이지 (Visitor View)"]
        ...
        subgraph Left ["좌측 영역 (프로필 & 브랜딩)"]
            ...
            A2["[ Hello, I'm ] 태그"]
            A3["메인 타이틀 (고승희)"]
            A4["서브 타이틀 (Ko Seung Hee)"]
            A5["생년월일 태그 (2004.10.11)"]
        end
    ...
    subgraph Admin ["관리자 대시보드 (Admin View)"]
        ...
        subgraph Main ["메인 편집 영역 (항상 연필 아이콘 노출)"]
            ...
            subgraph Left_Edit ["좌측 영역 (인라인 편집)"]
                ...
                E_A4["생년월일 (2004.10.11) ✏️ 클릭하여 수정 -> [ 저장 ]"]
            end
