...
import { Link as LinkIcon, Plus, Trash2, ExternalLink, Globe, Loader2, Pencil, Save, BarChart3 } from "lucide-react";
...
import { collection, addDoc, serverTimestamp, onSnapshot, query, orderBy, deleteDoc, doc, updateDoc, getDocs, setDoc, where } from "firebase/firestore";
...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    if (!authLoading && !initialLoading && username) {
      if (user && user.email?.split('@')[0] === username) {
        setIsOwner(true);
        // If owner doesn't exist in DB yet, create it!
        if (portfolioOwnerId === "NOT_FOUND") {
          ...
          createProfile();
        } else if (portfolioOwnerId !== user.uid && portfolioOwnerId !== "NOT_FOUND") {
          // rare case: email prefix matches, but uid doesn't? 
          // Treat as owner based on email prefix, but data fetch depends on portfolioOwnerId
        }
      } else {
        setIsOwner(false);
      }
    }
  }, [user, portfolioOwnerId, authLoading, initialLoading, username]);

  // Subscribe to profile data
  useEffect(() => {
    if (!portfolioOwnerId || portfolioOwnerId === "NOT_FOUND") return;
  ...
  const handleLinkClick = (linkId: string) => {
    if (portfolioOwnerId && portfolioOwnerId !== "NOT_FOUND") {
  ...
  // Not owner and user not found
  if (portfolioOwnerId === "NOT_FOUND" && !isOwner) {
    return (
      <div className="min-h-screen bg-slate-50/50 dark:bg-zinc-950 flex flex-col items-center justify-center p-4 text-center gap-4">
        <Globe className="w-12 h-12 text-muted-foreground/50" />
        <h1 className="text-xl font-bold">페이지를 찾을 수 없습니다</h1>
        <p className="text-sm text-muted-foreground">존재하지 않거나 아직 생성되지 않은 링크 트리입니다.</p>
        <Link href="/">
          <Button className="mt-4" variant="outline">메인으로 돌아가기</Button>
        </Link>
      </div>
    );
  }
      ...
      <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-md border-b border-border/40 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {user ? (
            ...
            <Button size="sm" onClick={handleLogin} className="text-xs bg-slate-900 dark:bg-slate-50 text-white dark:text-slate-900 rounded-full px-4 hover:opacity-90">
              로그인
            </Button>
          )}
        </div>
        ...
        {profile ? (
          ...
          isOwner && (
            <div className="text-center py-10">
              <Loader2 className="w-6 h-6 text-[#5b5fc7] animate-spin mx-auto mb-2" />
              <p className="text-sm text-muted-foreground">프로필 설정 중입니다...</p>
            </div>
          )
        )}
